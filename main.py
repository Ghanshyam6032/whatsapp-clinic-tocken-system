...
from sqlalchemy import Column, Integer, String, ForeignKey, func, or_
...
app = FastAPI(title="Clinic Token System API")
...
def setup_database_and_admin():
    ...
    try:
        ...
        # SAFEGUARD: Remove Admin from Doctors Table to prevent WhatsApp/Queue crossover
...
@app.get("/auth/me")
def get_me(current_admin: AdminSystem = Depends(get_current_admin), db: Session = Depends(get_db)):
    clinic = db.query(Clinic).filter(Clinic.id == current_admin.clinic_id).first()
    return {
        "id": current_admin.id,
        "clinic_id": current_admin.clinic_id,
        "name": "System Admin",
        "clinic_name": clinic.name if clinic else "Clinic Portal",
        "email": current_admin.username,
        "role": "admin"
    }

# ==========================================
# 7. DOCTORS ENDPOINTS (Admin != Doctor)
# ==========================================
...
# ==========================================
# 9. WHATSAPP WEBHOOK (Unchanged/Safe)
# ==========================================
@app.get("/webhook")
